...
bt_status_t btif_a2dp_source_setup_codec(tBTA_AV_HNDL hndl) {
  ...
  if (status == BT_STATUS_SUCCESS) {
    ...
    } else if (codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV2
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV5
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LC3PLUS_HR
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_FLAC) {
      uint32_t bitrate = 0;
      bitrate = A2DP_GetTrackBitRate(p_codec_info);
      APPL_TRACE_DEBUG(LOG_TAG,"bitrate = %d", bitrate);
...
static uint32_t btif_a2dp_source_read_callback(uint8_t* p_buf, uint32_t len) {
  ...
  while (true) {
    ...
    if (btif_a2dp_source_is_hal_v2_supported()) {
#if AHIM_ENABLED
      bytes_read = btif_ahim_read(p_buf + bytes_offset, len_read);
#else
      bytes_read = bluetooth::audio::a2dp::read(p_buf + bytes_offset, len_read);
#endif
    } else {
      bytes_read = UIPC_Read(UIPC_CH_ID_AV_AUDIO, &event, p_buf + bytes_offset, len_read);
    }
...
void btif_a2dp_source_encoder_LHDC_user_data_update_req(
  ...
  int clen = pData->clen;
  btif_a2dp_source_encoder_LHDC_user_data_update_event(peer_address, data, clen);


  //TODO: Do it like btif_a2dp_source_encoder_user_config_update_req?
